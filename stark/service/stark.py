...
class Row(object):
    ...
    def __iter__(self):         # 变成可迭代的
        ...
        for item in self.data_list:     # item().quervset中的一个对象
            val = self.option.get_value(item)
            text = self.option.get_text(item)

            query_dict = self.query_dict.copy()
            query_dict.mutable = True       # query_dict 默认是不能修改的，True就可以被修改
            ...
            if not self.option.is_multi:  # 单选
                ...
                if str(val) in origin_value_list:
                    muti_val_list.remove(str(val))                      # 已经选，把自己去掉
                    query_dict.setlist(self.option.field, muti_val_list)
                    yield '<a class="active" href="?%s">%s</a>' % (query_dict.urlencode(), text)
                else:
                    muti_val_list.append(val)
                    query_dict.setlist(self.option.field, muti_val_list)
                    yield '<a href="?%s">%s</a>' % (query_dict.urlencode(), text)
...
class ChangeList(object):
    ...
    def __init__(self, config, queryset, q, search_list, page):
        ...
        self.page = page
        self.config = config
        self.action_list = [{'name': func.__name__, 'text': func.text} for func in config.get_action_list()]    # Action

        self.add_btn = config.get_add_btn()     # 添加按钮

        self.queryset = queryset

        self.list_display = config.get_list_display()   # 处理表格
...
class StarkConfig(object):
    ...
    def __init__(self, model_class, site, prev):
        ...
        self.request = None
        self.prev = prev
        self.back_condition_key = "_filter"          # 原地址

    def display_checkbox(self, row=None, header=False):     # 选项框
        if header:
            return "选择"
    ...
    def display_del(self, row=None, header=False):
        if header:
            return "删除"
        return mark_safe(
            '<a href="%s"><i class="fa fa-trash-o" aria-hidden="true"></i></a></a>' % self.reverse_del_url(row)
        )
    ...
    def display_edit_del(self, row=None, header=False):
        if header:
            return "操作"
        tpl = """
            <a href="%s"><i class="fa fa-edit" aria-hidden="true"></i></a></a> |
    ...
    def get_list_display(self):     # 预留钩子，可在子类自定制显示内容,例如根据用户权限不同显示不同内容
        val = []
        val.extend(self.list_display)
        val.append(StarkConfig.display_edit)            # 默认都带编辑和删除
    ...
    def get_model_form_class(self):
        ...
        :return:
        """
        if self.model_form_class:       # 实现自定制编辑的表单项
            return self.model_form_class

        class AddModelForm(forms.ModelForm):            # 实现自动生成表单以及表单的验证
            class Meta:
                print(self.model_class, '7889')
    ...
    def changelist_view(self, request):
        ...
        if request.method == 'POST':
            ...
            if action_name not in action_dict:      # 防止恶意执行其他函数
                return HttpResponse('非法请求')
            response = getattr(self, action_name)(request)      # 根据反射执行相应函数
            if response:
                return response
        # ########处理搜索#######
        ...
        cl = ChangeList(self, queryset, q, search_list, page)       # 调用封装函数
    ...
    def add_view(self, request):
        ...
        form = AddModelForm(request.POST)           # 表单验证
        if form.is_valid():
    ...
    def get_urls(self):
        ...
        urlpatterns = [
            ...
            re_path('^(?P<pk>\d+)/change/$', self.wrapper(self.change_view), name=self.get_change_url_name),
            re_path('^(?P<pk>\d+)/del/$', self.wrapper(self.delete_view), name=self.get_del_url_name)
        ]
    ...
    def reverse_list_url(self):                 # 反向生成URL
        # app_label = self.model_class._meta.app_label
        # model_name = self.model_class._meta.model_name
    ...
    def reverse_add_url(self):      # 反向生成URL
...
class AdminSite(object):
    ...
    def get_urls(self):
        ...
        for item in self._registry:
            ...
            if item.prev:
                temp = path('%s/%s/%s/' % (app_label, model_name, item.prev), (item.config.urls, None, None))
            else:
                temp = path('%s/%s/' % (app_label, model_name), (item.config.urls, None, None))
            urlpatterns.append(temp)

        return urlpatterns

    @property
    def urls(self):     # 动态生成URL
