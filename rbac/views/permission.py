...
def menu_list(request):
    ...
    # 找到可以成为菜单的权限的所有子权限
    node_permission_list = models.Permission.objects.filter(pid__in=permissions).order_by('-id').values(
        'id',
        'title',
        'url',
        'name',
...
def multi_permissions(request):
    ...
    destroy_formset = MultiPermissionFormSet(
        initial=[row for name, row in permission_dict.items() if name in destroy_name_list]
    )
...
def distribute_permissions(request):
    ...
    all_menu_list = []
    if rid:
        role_has_permissions = models.Role.objects.filter(id=rid, permissions__id__isnull=False).values('id', 'permissions')
